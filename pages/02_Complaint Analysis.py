...
tab1, tab2, tab3 = st.tabs(
    ["Distributions", "Response Times & Status", "Temporal Trends"]
)
...
with tab1:
    ...
    with col2:
        ...
        fig = px.bar(
            ...
            title="Top 10 Complaint Types occurring in NYC",
            labels={
    ...
    # MOVED THIS INSIDE TAB 1 - Complaint details chart
    st.markdown("### Top 3 Complaint Details Within Top 5 Complaint Types")
...
with tab2:
    ...
    with col3:
        ...
        plot_df = (
            df.groupby("complaint_type")["response_time_hours"]
            .mean()
            .round(2)
            .reset_index()
            .sort_values(by="response_time_hours", ascending=False)
            .head(10)
        )
        fig = px.bar(
            plot_df,
            ...
            y="complaint_type",
            title="Average Response Time by Complaint Type",
            labels={
                "response_time_hours": "Average Response Time (Hours)",
                "complaint_type": "Complaint Type",
            },
            color_discrete_sequence=[THEME_COLORS[0]],
            text_auto=True,
    ...
    fig = px.pie(
        ...
        names="status",
        values="count",
        title="Distribution of Complaint Statuses",
        color_discrete_sequence=THEME_COLORS,
    )
    fig.update_layout(
        plot_bgcolor="rgba(0,0,0,0)",
...
with tab3:
    ...
    plot_df = (
        df.groupby("created_date")["complaint_type"].count().reset_index(name="count")
    )
    fig = px.line(
        plot_df,
        x="created_date",
        ...
        labels={
            "count": "Number of Complaints",
            "created_date": "Complaint Created Date",
        },
        color_discrete_sequence=[THEME_COLORS[0]],
    )
    fig.update_layout(
