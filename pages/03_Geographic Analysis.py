...
with tab1:
    ...
    fig = px.bar(
        ...
        text_auto=True,
        title="Top 15 streets with the highest complaint volume",
...
with tab2:
    ...
    plot_df = (
        df.groupby("borough")["response_time_days"]
        .mean()
        .astype("int")
        .reset_index()
        .sort_values(by="response_time_days")
    )
    ...
    fig = px.bar(
        ...
        title="Average Response Time in Days",
        labels={
    ...
    # Reporting channel differ across boroughs
    st.markdown("### Reporting Channel (Phone/Mobile/Online) Differ Across Boroughs")
    plot_df = (
        df.groupby(["borough", "open_data_channel_type"])
        .size()
